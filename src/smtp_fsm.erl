...
%% ok
%% 
%% =ERROR REPORT==== 24-Nov-2003::15:37:20 ===
%% ** State machine <0.45.0> terminating 
...
-export([handle_event/3, handle_sync_event/4, init/1, handle_info/3, code_change/4,
	 terminate/3]).
...
smtp_conn({login_login,User}, _Pid, Info) ->
    B64Usr =  base64:encode_to_string(User),
...
smtp_md5({md5_send, User, Md5_hmac}, _Pid, Info) ->
    Md5Usr = base64:encode_to_string(User++" "++Md5_hmac),
...
smtp_login({login_pass,Pwd}, _Pid, Info) ->
    Msg = [base64:encode_to_string(Pwd), "\r\n"],
    ok = gen_tcp:send(Info#info.socket, Msg),
    case get_response(Info#info.socket) of
	{"235", Resp} -> 
	    {reply, {ok, Resp}, smtp_conn, Info};
